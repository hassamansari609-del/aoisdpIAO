...
export default function AdminListingsPage() {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
      ...
      if(!error) {
          // 2. If rejected/active, we might want to notify user (not implemented yet)
              ...
              <table className="min-w-full divide-y divide-gray-300">
                <thead>
                  <tr>
                    ...
                    <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Proof</th>
                  ...
                  {listings.map((listing) => (
                    <tr key={listing.id}>
                      <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0">
                          {listing.title}
                          <div className="text-xs text-gray-500">Seller: {listing.seller_id}</div>
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {listing.proof_image_url ? <a href={listing.proof_image_url} target="_blank" className="text-indigo-600 hover:underline">View</a> : 'None'}
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {/* Admin can see vault for verification purposes */}
                          <div className="max-w-xs truncate" title={listing.credentials_vault}>{listing.credentials_vault}</div>
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${
                          listing.status === 'active' ? 'bg-green-50 text-green-700 ring-green-600/20' :
                          listing.status === 'pending_approval' ? 'bg-yellow-50 text-yellow-800 ring-yellow-600/20' :
